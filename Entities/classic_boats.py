...
class TrapperBoat(Boat):
    ...
    def __init__(self, team_name: str, position: Vector2, lane: int, asset_manager: AssetManager):
        ...
        # Main Data
        self.position: Vector2 = position  # Position (Centered)
        self.rect: Rect = Rect((*position, 110, 70))  # X Y Position, Width, Height
        self.rect_offset: Vector2 = Vector2(0, 6)  # Offset for rect (Applied in self.fix_other_positions)
        self.texture_offset: Vector2 = Vector2(0, -10)  # Offset for texture rendering
        self.lane: int = lane  # Which lane (1/2/3) it's on
        self.sprite: Sprite2D = Sprite2D(  # Boat Sprite
            texture=asset_manager.get("textures", f"{self.texture_id}{team_name.upper()}"),  # Get Texture
            position=self.position,  # Sprite should always follow self.position
        ...
        self.health_max: int = 200  # Boat's Max Health
        self.health: int = self.health_max  # Boat's Health
        self.speed: int = 25  # Boat's Speed in px/s calculated by (speed*dt). Can be -/+ Depending on Team
        self.damage: int = 75  # Boat's Damage (When collision, damage is exchanged twice. Technically 25==50DMG)
        self.fix_direction()  # Fix the Boat's direction immediately
    ...
    def update(self, dt: float, teams, team_edges, asset_manager=None, mixer=None):
        self.update_basic(dt, teams, team_edges)  # BORING necessary basic stuff
        # Trapping updated outside

    def update_trapping(self, dt, asset_manager, mixer):
        # Shooting
        if self.trap_timer.update(dt) and asset_manager is not None:  # Can shoot
            mixer.play_sound("effects/placetrap")
            return Trap(self.position + self.trap_position_offset,
                                               -30 if self.speed > 0 else 30, -50,
                                               self.position.y + 40,
                                               self.team_name, self.lane, asset_manager, mixer)
        return None
